...
func (o *ClusterUninstaller) deleteVPCs() error {
	...
	err := pager.EachPage(func(page hwpagination.Page) (bool, error) {
		...
		for _, v := range vpcList {
			o.Logger.Debugf("Getting tags for VPC %s", v.ID)
			...
			if IsMapSubset(vpcTagsMap, o.CommonTags) {
				o.Logger.Debugf("Deleting VPC %s", v.ID)

				err = hwvpcs.Delete(o.NetworkClientV1, v.ID).ExtractErr()
				if err != nil {
					return false, err
				}
			} else {
				o.Logger.Debugf("Leaving VPC %s as tags don't match", v.ID)
			}
		}
